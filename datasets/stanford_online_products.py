...
class StanfordOnlineProducts(Dataset):
    ...
    def load_labels(self):
        ...
        for k, v in info_files.items():
            curr_file = read_csv('%s/%s'%(self.dataset_folder,v), delim_whitespace=True, header=0).values
            self.img_paths.extend(['%s/%s'%(self.dataset_folder, x) for x in list(curr_file[:,3])])
