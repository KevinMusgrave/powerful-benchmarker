...
def already_done(exp_path, config_names):
    ...
    for c in config_names:
        full_path = os.path.join(exp_path, c)
...
def base_command(exp_name, adapter, exp_path, cfg):
    x = f"python main.py --exp_name {exp_name} --adapter {adapter} --start_with_pretrained \
...
def exp_launcher(cfg, exp_path, exp_names):
    ...
    command = base_command(dir_name, config_name, exp_path, cfg)
    full_command = f"bash -i ./scripts/{cfg.script_wrapper} {dir_name} {str(cfg.script_wrapper_timeout)} {exp_path} {cfg.conda_env} {use_devices} {BEST_TRIAL_FILENAME}".split(
        " "
    )
...
def main(cfg, slurm_args):
    ...
    executor = submitit.AutoExecutor(folder=os.path.join(exp_path, "slurm_logs"))
